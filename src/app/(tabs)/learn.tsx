...
export default function LearnScreen() {
  ...
  // Fallback to Spanish if selectedLanguage is not set
  ...
  const getLessonStatus = (lesson: Lesson, index: number) => {
    ...
    // If not completed, find if it's the current in-progress lesson (first non-completed or index === 1)
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View className="flex-row items-center justify-between px-5 py-3 border-b border-slate-100 bg-white">
        ...
        <TouchableOpacity
          className="h-10 w-10 items-center justify-center rounded-full bg-[#FF9500] shadow-sm active:opacity-90"
          activeOpacity={0.8}
        >
      ...
      >
        {/* Hero Scenery Banner */}
        <View className="relative bg-gradient-to-r from-lingua-purple to-lingua-deep-purple bg-[#6C4EF5] rounded-3xl p-5 mb-5 overflow-hidden shadow-md">
          ...
          <View className="flex-row items-center justify-between">
            <View className="flex-1 pr-3">
              <View className="bg-white/20 self-start px-3 py-1 rounded-full mb-2">
                <Text className="font-['Poppins-SemiBold'] text-xs text-white uppercase tracking-wider">
                  {currentLang.flag} {currentLang.name}
                </Text>
              </View>
              <Text className="font-['Poppins-Bold'] text-xl text-white leading-tight">
                {activeUnit ? activeUnit.title : "Start Learning"}
              </Text>
              <Text className="font-['Poppins-Regular'] text-xs text-white/90 mt-1">
                ...
                >
                  ...
                  <View className="flex-1 pr-2">
                    <View className="flex-row items-center gap-2 mb-1">
                      <Text className="font-['Poppins-Bold'] text-base text-text-primary leading-tight">
                        {lesson.title}
                      </Text>
                    </View>

                    <Text className="font-['Poppins-Regular'] text-xs text-text-secondary" numberOfLines={1}>
                      {lesson.description}
                    </Text>
                    ...
                    <View className="flex-row items-center mt-2 gap-3">
                      {/* Status Tag Badge */}
                      {isCompleted ? (
                        <View className="bg-emerald-100/70 px-2.5 py-0.5 rounded-full">
